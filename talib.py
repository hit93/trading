...
def ADX(df, n, n_ADX):  
    ...
    while i + 1 <= df.index[-1]:  
        ...
        DoMove = df.loc[i, 'Close'] - df.loc[i + 1, 'Close']  
    ...
    TR_l = [0]  
    while i < df.index[-1]:  
        TR = max(df.loc[i + 1, 'High'], df.loc[i, 'Close']) - min(df.loc[i + 1, 'Close'], df.loc[i, 'Close'])  
        TR_l.append(TR)  
        i = i + 1  
    TR_s = pd.Series(TR_l)  
...
def RSI(df, n):  
    ...
    UpI = pd.Series(UpI)  
    DoI = pd.Series(DoI)  
    PosDI = pd.Series(UpI.ewm(span = n).mean()) 
    NegDI = pd.Series(DoI.ewm(span = n).mean())  
    RSI = pd.Series((100-100/(1+PosDI/NegDI)), name = 'RSI_' + str(n))  
    df = df.join(RSI)  
    return df
